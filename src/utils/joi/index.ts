...
const UpdateOrder = Joi.object({
    ...
    status: Joi.string().optional().valid(...Object.keys(OrderStatus)),
    orderItems: Joi.array().items(OrderItem).optional().min(1),
    shippingAddress: ShippingAddress,
    paymentId: Joi.string().optional(),
})

export function validateCreateOrder(data: unknown, options: Joi.AsyncValidationOptions = { abortEarly: false }) {
    return CrteateOrder.validateAsync(data, options);
}

export function validateUpdateOrder(data: unknown, options: Joi.AsyncValidationOptions = { abortEarly: false }) {
