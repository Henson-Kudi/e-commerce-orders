...
export default class DeleteOrdersUseCase implements IUseCase<FindOrdersFilter, Promise<IReturnValue<{ count: number }>>> {
    constructor(private readonly ordersRepository: IOrdersRepository, private readonly messageBroker: IMessageBroker) { }

    async execute(params: FindOrdersFilter): Promise<IReturnValue<{ count: number; }>> {
